class ANote extends Note{
  ...
  int atTarget(Target t){
    ...
    if((distance <= 10) && (distance >2) && keyPressed && key=='a'){
      return 1; //1000
    } 

    else if((distance <= 20) && (distance >10) && keyPressed && key=='a'){
      return 2; // 500
    } 
    
    else if((distance < 10) && (distance >2) && keyPressed && key!='a'){
      return 0;
      // wrong key 
    }
